...
import com.jinggan.library.media.picture.commpressor.Luban;
import com.jinggan.library.media.picture.commpressor.OnCompressListener;
        ...
        if (!TextUtils.isEmpty(mList.get(position).getNewUrl()) && mList.get(position).getNewUrl().startsWith(ADD_IMG_FLAG)) {
            ...
            if (!TextUtils.isEmpty(mList.get(position).getNewUrl())) {
                if ("compressing".equals(mList.get(position).getNewUrl())) {
                    Glide.with(mContext).load(R.mipmap.default_image).into(hodler.imageView);
                } else {
